        ...
        for attempt_index in range(1, max_attempts + 1):
            ...
            if allow_surface_alignment and not surface_alignment_attempted:
                ...
                if alignment.get("status") == "blocked":
                    attempt["surface_alignment_fallback"] = {
                        "status": "continuing_without_surface_alignment",
                        "reason": (
                            "No footprint-clear support-surface standoff was available; "
                            "continuing with visual centering and forward approach from the current pose."
                        ),
                    }
            center = _detection_center_error(selected, capture)
