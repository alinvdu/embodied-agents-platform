...
- after a match, `focus_detected_object` reuses the selected bbox to recenter the object, `approach_detected_object` first tries to align the robot body perpendicular to the occupied support surface, relocalizes after that alignment, then uses that tracked bbox with RGB-D depth plus tiny safe forward steps until the object is around `0.35-0.45m`; `grab_object` returns a mocked VLA entrypoint result
